...
const moduleResources = [
	...
	// Fetch API polyfill for jsdom-worker and wasm-imagemagick.
	{
		moduleName: 'whatwg-fetch',
		directoryName: 'dist',
		filename: 'fetch.umd.js'
	},
...
<html>
	...
	<script src="http://localhost/mitt.umd.js"></script>
	<script src="http://localhost/fetch.umd.js"></script>

	<!-- Add interceptor to provide the webworker and wasm files. -->
	<script>
		const originalFetch = window.fetch;
		window.fetch = window.fetchInterceptor.bind(null, originalFetch);
	</script>

	<script src="http://localhost/jsdom-inline-worker.umd.js"></script>
	<script src="http://localhost/wasm-imagemagick.umd-es5.js"></script>
	</body>
</html>
`;
...
async function createImageMagickInstance(options = {}) {
	...
	const dom = new jsdom.JSDOM(html, {
		...
		virtualConsole,
		resources,
		// Attach an interceptor to the Fetch API to provide the ImageMagick
		// webworker and wasm files.
		beforeParse: window => {
			window.fetchInterceptor = createFetchInterceptor(config)
		}
	});
...
function createFetchInterceptor(config) {
	 ...
	 */
	const fetchInterceptor = (originalFetch, input, init) => {
		if (config.enableRequestLogging) {
